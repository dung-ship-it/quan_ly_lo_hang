...
// NEW: track 1 row being edited to avoid saving other "chua_luu" rows accidentally
let editingIdx = null;
...
function renderTable() {
    ...
    // show save button if any unsaved OR currently editing a row
    $('#btnLuuNgay').toggle(coTrang || editingIdx !== null);
...
// NEW: stronger selected matching
function renderKhachHangOptions(selectedId) {
    const sid = parseInt(selectedId || 0, 10);
    return KHACH_HANG_LIST.map(kh =>
        `<option value="${kh.id}" ${parseInt(kh.id, 10) === sid ? 'selected' : ''}>${kh.ma_khach}</option>`
    ).join('');
}
...
function onFieldChange(idx) {
    // only mark unsaved for the row currently editable
    if (loHangData[idx]) {
        if (loHangData[idx].trang_thai !== 'chua_luu') return;
    }
    hasUnsaved = true;
    $('#btnLuuNgay').show();
}
...
function themLoHang() {
    editingIdx = loHangData.length; // new row being edited
    const newLo = {
...
function suaLoHang(idx) {
    // NEW: only focus editing on this row to avoid saving other rows accidentally
    editingIdx = idx;
    ...
    showConfirm('Xoá lô hàng?', 'Bạn có chắc muốn xoá lô hàng này?', function() {
        if (lh.id) {
            ...
            apiCall(BASE_URL + 'api/lo_hang/delete.php', { id: lh.id }, function(res) {
                Swal.close();
                if (res.success) { loHangData.splice(idx, 1); editingIdx = null; renderTable(); }
...
function luuNgay() {
    ...
    loHangData.forEach((lh, idx) => {
        if (lh.trang_thai === 'chua_luu') {
            // NEW: if editing a specific row, save only that row
            if (editingIdx !== null && idx !== editingIdx) return;
    ...
    toSave.forEach(item => {
        ...
        $.ajax({
            ...
            processData: false,
            contentType: false,
            dataType: 'json', // NEW: ensure JSON parsing
            success: function(res) {
                ...
                if (saved === toSave.length) {
                    ...
                    if (lastRes && lastRes.success) {
                        ...
                        showError((lastRes && lastRes.message) ? lastRes.message : 'Lỗi khi lưu!');
                    }
                }
            },
            error: function(xhr) {
                Swal.close();
                showError(xhr.responseText || 'Lỗi khi lưu!');
            }
        });
    });
}
